...
#include <Adafruit_BME280.h>
        ...
        int postTemp;//                 Variable de comparaison changement température
        int tempCall = (60)*1000;//     Delai 60 secondes évite pompage variation temp 
        unsigned long lastTempCall;
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
     ...
     Serial.println(len);
//   Extraction retourAction pour vérification et consigne pour modif par Télégram Bot
     retourAction = espnowDatas.valideAction; Serial.println(retourAction);
     consigne = espnowDatas.tempConsigne; Serial.println(consigne);
...
//   Pour forcer un nouvel affichage lors changement consigne par Télégram Bot
     Changement = true;
...
//   Vérifie toutes les minutes si changement de température si oui envoi espnowDatas pour mise à jour temp
     if ((millis() - lastTempCall) > tempCall){
         lastTempCall = millis();
         if (postTemp != int(temp)) { Changement = true; postTemp = int(temp); Serial.println("Delta température relevé"); }
        }
...
//   Et positionnement Flag Changement pour nouvelle action à destination du récepteur      
     ...
     if ((int(temp) == consigne && valide == true) || Changement == true) {
         choixAction = 'Z';
...
//   Envoi de la structure espnowDatas si Flag Changement est vrai 
     if (Changement == true) { 
         ...
         espnowDatas.tempConsigne = consigne; Serial.println(consigne); delay(10);
         espnowDatas.Temperature = temp; Serial.println(temp); delay(10);
         espnowDatas.Action = choixAction; Serial.println(char(choixAction)); delay(10);
